...
module.exports = function(pathToZip, options) {
  ...
  var prepFilePath = function(originUrl, filePath) {
    try {
      var arr = originUrl.split("/").map(s => decodeURIComponent(s))
      ...
      var arr2 = filePath.split("/").map(s => decodeURIComponent(s))
  ...
  return function(req, res, next) {
    ...
    // Strip the leading '/'
    var baseName = prepFilePath(req.originalUrl, req.path)
    if (baseName.endsWith("/")) {
      baseName = baseName.substring(0, baseName.length - 1)
    }
    var candidateNames = [baseName, baseName + "/index.html", "index.html"]
    for (let name of candidateNames) {
      ...
      if (zipDir.hasOwnProperty(name)) {
        ...
        return res.status(200).send(entryData)
      }
    }
    return next()
  }
